...
abstract class Page
{
    ...
    /**
     * Generates a single item in the navbar
     * 
     * @param $link   The link of the site this item references
     * @param $title  The title of the site this item references
     * @param $currentPageName Name of the current page. Used to determine 'active' navbar items
     */
    private function generateNavbarItem(string $link, string $title, string $currentPageName)
    {
        $class = $title == $currentPageName ? " active" : "";

        echo <<< EOT
        <a class="navbar-link$class" href="$link">$title</a>
    ...
    protected function generatePageHeader(string $title = ""):void
    {
        ...
        <nav class="navbar">
            <img alt="" height=48 src="img/logo.svg">
        ...
        EOT;
            ...
            $this->generateNavbarItem("index.php", "Home", $title);
            $this->generateNavbarItem("order.php", "Bestellung", $title);
            $this->generateNavbarItem("customer.php", "Kunde", $title);
            if ($this->DEBUG_MODE) {
                $this->generateNavbarItem("baker.php", "Bäcker", $title);
                $this->generateNavbarItem("driver.php", "Fahrer", $title);
            }
